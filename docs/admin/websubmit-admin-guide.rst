...
Specific topics would gain to be more developed, such as HOW-TOs,
...
One thing worth to learn from this simple workflow is that (1) functions
...
Finally note that you can plug check on each field of a page, so that
the user cannot proceed further if some invalid text has been input.
...
provide all the necessary functionalities to create your own workflow.
...
WebSubmit engine more or less limits to 1) displaying page to collect
...
and 3) let the users chose among the available "categories" (described
further below).
