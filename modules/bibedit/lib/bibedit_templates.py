...
class Template:
        ...
        if type_table != "record" or add == 3:
            ...
            if add == 1 or add == 3:
                ...
                if len(value) < 75:
                    ...
                    print_value = """ <textarea name=\"value%(num_value)s\" cols=\"70\" rows=\"5\" style=\"width:100%(pourcentage)c;\">%(value)s</textarea>  """ % {'num_value'   : str(num_value),
                                         'pourcentage' : '%',
                                         'value'       : value}
    ...
    def tmpl_link(self, ln, text, url, dest, dict_args='', ancre=''):
        ...
        return  """ <a href=\"%(url)s/%(dest)s%(args)s%(ancre)s\"> %(text)s </a>
                """ % {'text'  : text,
                       'url'   : url,
                       'dest'  : dest,
                       'args'  : link_args,
