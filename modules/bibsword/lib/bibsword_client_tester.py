...
class Test_format_marcxml_file(unittest.TestCase):
   ...
   def test_extract_marcxml_1(self):
      ...
      self.assertEqual('id' in metadata, False)
      ...
      self.assertEqual(metadata['journal_refs'][0], "J. Phys.: Conf. Ser. 160 (2009) 012061")
      self.assertEqual(len(metadata['journal_refs']), 1)
      self.assertEqual('report_nos' in metadata, True)
      self.assertEqual('comment' in metadata, False)
   ...
   def test_extract_marcxml_2(self):
      ...
      self.assertEqual('doi' in metadata, False)
      self.assertEqual(metadata['journal_refs'][0], "JINST 5 (2010) P04003")
      self.assertEqual(len(metadata['journal_refs']), 1)
      self.assertEqual('report_nos' in metadata, True)
   ...
   def test_extract_full_marcxml_3(self):
      ...
      self.assertEqual('doi' in metadata, False)
      self.assertEqual(metadata['journal_refs'][0], "Nucl. Phys. B, Proc. Suppl. 177-178 (2008) 55-59")
      self.assertEqual(len(metadata['journal_refs']), 1)
...
class Test_format_metadata(unittest.TestCase) :
   ...
   def test_correct_metadata_collection(self) :
      """Test_format_metadat - collection of metadata without errors"""
      ...
      metadata = format_metadata(4, marcxml, deposit_results, user_info,
                                 metadata)
      ...
      self.assertEqual(metadata['links'][0]['link'], "https://arxiv.org/sword-app/edit/10070072")
      self.assertEqual(metadata['links'][0]['type'], "application/pdf")
...
class Test_swrDATA_table(unittest.TestCase) :
   '''
      This test check that the entire update process works fine. It insert some
      data into the swrDATA table, then he get some xml status entry from
   ...
   def test_insert_submission(self) :
      '''Test_insert_submission - check insert submission rows in swrDATA'''

      self.id_tests.append(insert_into_swr_clientdata(4, 97, 1, 'test', 'test', '10030148',
                               'https://arxiv.org/sword-app/edit/10030148',
                               'https://arxiv.org/sword-app/edit/10030148.atom',
                               'http://arxiv.org/resolve/app/10030148'))

      self.id_tests.append(insert_into_swr_clientdata(4, 92, 2, 'test', 'test', '10070221',
                               'https://arxiv.org/sword-app/edit/10070221',
                               'https://arxiv.org/sword-app/edit/10070221.atom',
                               'http://arxiv.org/resolve/app/10070221'))

      self.id_tests.append(insert_into_swr_clientdata(3, 92, 2, 'test', 'test', '12340097',
      ...
      for row in rows :
         ...
         if row[0] == self.id_tests[1] :
            ...
            self.assertEqual(row[3], '10070221')
   ...
   def test_yread_submission(self) :
      '''Test_insert_submission - check read submission rows in swrDATA'''
      ...
      self.assertEqual(len(results), run_sql('''SELECT COUNT(*) FROM swrDATA''')[0][0])
      for result in results :
         ...
         if result['id'] == self.id_tests[0] :
            ...
            self.assertEqual(result['id_user'], 1)
            ...
            self.assertEqual(result['linkMedias'], 'https://arxiv.org/sword-app/edit/10030148')
            self.assertEqual(result['linkMetadata'], 'https://arxiv.org/sword-app/edit/10030148.atom')
            self.assertEqual(result['linkStatus'], 'http://arxiv.org/resolve/app/10030148')
            self.assertEqual(result['status'], CFG_SUBMISSION_STATUS_SUBMITTED)

      results = select_submitted_record_infos(4)

      self.assertEqual(len(results), run_sql('''SELECT COUNT(*) FROM swrDATA WHERE idSwrREMOTESERVER=4''')[0][0])
      ...
      for result in results :
         ...
         if result['id'] == self.id_tests[1] :
            ...
            self.assertEqual(result['id_user'], 2)
            self.assertEqual(result['id_remote'], '1007.0221')
            self.assertEqual(result['submission_date'], currentDate)
            self.assertEqual(result['publication_date'], currentDate)
            self.assertEqual(result['removal_date'], "")
            self.assertEqual(result['linkMedias'], 'https://arxiv.org/sword-app/edit/10070221')
            self.assertEqual(result['linkMetadata'], 'https://arxiv.org/sword-app/edit/10070221.atom')
            self.assertEqual(result['linkStatus'], 'http://arxiv.org/resolve/app/10070221')
            self.assertEqual(result['status'], CFG_SUBMISSION_STATUS_PUBLISHED)


      results = select_submitted_record_infos(3, self.id_tests[2])
      self.assertEqual(len(results), 1)

      for result in results :
         self.assertEqual(result['id'] in self.id_tests, True)
         if result['id'] == self.id_tests[2] :
            self.assertEqual(result['id_server'], 3)
            self.assertEqual(result['id_record'], 92)
            self.assertEqual(result['id_user'], 2)
            ...
            self.assertEqual(result['linkMedias'], 'https://arxiv.org/sword-app/edit/12340097')
            self.assertEqual(result['linkMetadata'], 'https://arxiv.org/sword-app/edit/12340097.atom')
            self.assertEqual(result['linkStatus'], 'http://arxiv.org/resolve/app/12340097')
