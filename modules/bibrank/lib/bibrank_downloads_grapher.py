...
def draw_downloads_statistics(id_bibrec, id_bibdoc_list):
    ...
    intervals = create_tic_interval(local_time, creation_date_year, creation_date_month)    
    if id_bibdoc_list == []:
        ...
        for i in range(len(id_bibdoc_list)):
            datas = create_list_tuple_data(creation_date_day, intervals, id_bibrec, id_bibdoc="and id_bibdoc=%s" % id_bibdoc_list[i][0])
...
def create_list_tuple_data(creation_date_day, intervals, id_bibrec, id_bibdoc=""):
    ...
    for i in range(len(intervals)-1):
        ...
        res = run_sql("select count(*) from rnkDOWNLOADS where id_bibrec=%s %s and download_time>='%s' and download_time<'%s';" \
                      % (id_bibrec, id_bibdoc, date1, date2))[0][0]
...
def sort_list_tuple_by_date(list_tuple):
    ...
    list_tuple.sort(lambda x, y: (cmp(string.split(x[0], '/')[1], string.split(y[0], '/')[1])))
    return list_tuple
...
def create_tic_interval(local_time, creation_date_year, creation_date_month):
