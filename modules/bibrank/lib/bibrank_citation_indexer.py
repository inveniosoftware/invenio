...
#pylibdir = "/opt/cdsware-PCUDS11/lib/python"
...
class memoise:
	...
	def __call__(self, *args):
		if self.memo.has_key(args):
                    ...
                    return object
def get_record_id(pvalue, fvalue):
    rec_id = search_pattern(p=pvalue, f=fvalue, m='e').tolist()
    return rec_id
get_record_id = memoise(get_record_id)
...
def get_citation_weight(rank_method_code, config):
        ...
        if last_modified_records:
                ...
                print citation_weight_dic_intermediate[339705],citation_list_intermediate[339705],reference_list_intermediate[339705]
...
def create_recordid_list(rec_ids):
    """Create a list of record id. The result is expected to have ascending numerical order.
    """
    rec_list = []
    for row in rec_ids:
...
def ref_analyzer(citation_informations, initialresult, initial_citationlist, initial_referencelist):
    ...
    print 'result: ',result[339705]
    print citation_list[339705],str(len(citation_list[339705]))
    print reference_list[339705],str(len(reference_list[339705]))
