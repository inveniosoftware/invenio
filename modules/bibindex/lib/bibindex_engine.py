...
def get_associated_subfield_value(recID, tag, value, associated_subfield_code):
    ...
    if field_number > 0:
        for row in res:
            if row[0] == field_number and \
               row[1] == tag[:-1] + associated_subfield_code:
                out = row[2]
                break
    return out
...
def swap_temporary_reindex_tables(index_id, reindex_prefix="tmp_"):
    ...
    run_sql(
        "RENAME TABLE " +
        "idxWORD%02dR TO old_idxWORD%02dR,"
        % (index_id, index_id) +
        "%sidxWORD%02dR TO idxWORD%02dR,"
        % (reindex_prefix, index_id, index_id) +
        "idxWORD%02dF TO old_idxWORD%02dF,"
        % (index_id, index_id) +
        "%sidxWORD%02dF TO idxWORD%02dF,"
        % (reindex_prefix, index_id, index_id) +
        "idxPAIR%02dR TO old_idxPAIR%02dR,"
        % (index_id, index_id) +
        "%sidxPAIR%02dR TO idxPAIR%02dR,"
        % (reindex_prefix, index_id, index_id) +
        "idxPAIR%02dF TO old_idxPAIR%02dF,"
        % (index_id, index_id) +
        "%sidxPAIR%02dF TO idxPAIR%02dF,"
        % (reindex_prefix, index_id, index_id) +
        "idxPHRASE%02dR TO old_idxPHRASE%02dR,"
        % (index_id, index_id) +
        "%sidxPHRASE%02dR TO idxPHRASE%02dR,"
        % (reindex_prefix, index_id, index_id) +
        "idxPHRASE%02dF TO old_idxPHRASE%02dF,"
        % (index_id, index_id) +
        "%sidxPHRASE%02dF TO idxPHRASE%02dF;"
        % (reindex_prefix, index_id, index_id)
    )
    write_message("Dropping old index tables for id %s" % index_id)
    run_sql_drop_silently(
        """
        DROP TABLE old_idxWORD%02dR,
            old_idxWORD%02dF,
            old_idxPAIR%02dR,
            old_idxPAIR%02dF,
            old_idxPHRASE%02dR,
...
def init_temporary_reindex_tables(index_id, reindex_prefix="tmp_"):
    ...
    run_sql("""CREATE TABLE %sidxWORD%02dR (
                ...
                termlist longblob,
                type enum('CURRENT','FUTURE','TEMPORARY')
...
def detect_tokenizer_type(tokenizer):
    ...
    from invenio.bibindex_tokenizers.BibIndexRecJsonTokenizer import \
...
class AbstractIndexTable(object):
    ...
    def __init__(self, index_name, table_type, table_prefix="",
                 wash_index_terms=50):
        self.index_name = index_name
        self.index_id = get_index_id_from_index_name(index_name)
        self.table_type = table_type
        self.wash_index_terms = wash_index_terms
        self.table_name = wash_table_column_name(
            table_prefix + "idx" + table_type + ("%02d" % self.index_id) + "F"
        )
    ...
    def put_into_db(self, mode="normal"):
        ...
        for word in self.value.keys():
            ...
            nb_words_done += 1
            if nb_words_report != 0 and \
               (nb_words_done % nb_words_report) == 0:
                write_message('......processed %d/%d words' %
                              (nb_words_done, nb_words_total))
                percentage_display = get_percentage_completed(
                    nb_words_done, nb_words_total)
    ...
    def put_word_into_db(self, word):
        ...
        recids = self.load_old_recIDs(word)
        if recids is not None:  # merge the word recIDs found in memory:
            hitlist_was_changed = self.merge_with_old_recIDs(word, recids)
            if not hitlist_was_changed:
                ...
                write_message("......... updating hitlist for ``%s''" %
                              word, verbose=9)
                run_sql(
                    "UPDATE %s SET hitlist=%%s WHERE term=%%s"
                    % wash_table_column_name(self.table_name),
                    (recids.fastdump(), word)
                )  # kwalitee: disable=sql

        else:  # the word is new, will create new recids:
            write_message("......... inserting hitlist for ``%s''" %
                          word, verbose=9)
            recids = intbitset(self.value[word].keys())
            try:
                run_sql(
                    "INSERT INTO %s (term, hitlist) VALUES (%%s, %%s)" %
                    wash_table_column_name(self.table_name),
                    (word, recids.fastdump())
                )  # kwalitee: disable=sql
            except Exception as e:
                # We send this exception to the admin only when is not
                # already reparing the problem.
                register_exception(
                    prefix="Error when putting the term '%s' "
                           "into db (hitlist=%s): %s\n"
                           % (repr(word), recids, e),
                    alert_admin=(task_get_option('cmd') != 'repair')
                )

        if not recids:  # never store empty words
            run_sql(
                "DELETE FROM %s WHERE term=%%s"
                % wash_table_column_name(self.table_name),
                (word,)
            )  # kwalitee: disable=sql
    ...
    def merge_with_old_recIDs(self, word, recids):
        ...
        in the database index and received in recids universe
        of recIDs for the given word.

        Return False in case no change was done to SET, return True in case SET
        was changed.
        """
        oldset = intbitset(recids)
        recids.update_with_signs(self.value[word])
        return recids != oldset
...
class VirtualIndexTable(AbstractIndexTable):
    ...
    def __init__(self, index_name, table_type, table_prefix="",
        ...
        AbstractIndexTable.__init__(self, index_name,
                                    ...
                                    wash_index_terms)
        self.mode = "normal"
        self.dependent_indexes = dict(
            get_virtual_index_building_blocks(self.index_id))
        ...
        if self.mode == "reindex":
            ...
            for index_id, index_name in self.dependent_indexes.iteritems():
                ...
                if not rng:
                    continue
                write_message(
                    'Virtual index: %s is being reindexed for %s index'
                    % (self.index_name, index_name)
                )
            ...
            for index_id, index_name in self.dependent_indexes.iteritems():
                query = """SELECT id_bibrec_low, id_bibrec_high, mode FROM %s
                    ...
                    (self.table_name[:-1] + "Q")
                entries = self.remove_duplicates(
                    run_sql(query, (index_name, )))
                if entries:
                    write_message(
                        'Virtual index: %s is being updated for %s index'
                        % (self.index_name, index_name)
                    )
    ...
    def retrieve_new_values_from_index(self, index_id, records_range):
        ...
        new_regular_values = run_sql(
            query, (records_range[0], records_range[1]))
        if new_regular_values:
            zipped = zip(*new_regular_values)
            new_regular_values = dict(
                zip(zipped[0], map(deserialize_via_marshal, zipped[1])))
        else:
            new_regular_values = dict()
    ...
    def update_index(self, index_id, recID_low, recID_high):
        ...
        # take new values
        new_regular_values = self.retrieve_new_values_from_index(
            index_id, [recID_low, recID_high])
        ...
        for recID in xrange(recID_low, recID_high + 1):
            ...
            to_serialize = update_cache_for_record(
                index_name, recID, old_values, new_values)
            if len(to_serialize) == 0:
                continue
            run_sql(
                """INSERT INTO %s (id_bibrec,termlist,type)
                   VALUES (%%s,%%s,'FUTURE')""" %
                wash_table_column_name(virtual_tab_name),
                (recID, serialize_via_marshal(to_serialize))
            )  # kwalitee: disable=sql
            try:
                run_sql(
                    """INSERT INTO %s (id_bibrec,termlist,type)
    ...
    def insert_index(self, index_id, recID_low, recID_high):
        ...
        for recID in xrange(recID_low, recID_high + 1):
            new_values = new_regular_values.get(recID) or []
            old_values = old_virtual_values.get(recID) or []
            to_serialize = insert_to_cache_for_record(
                index_name, recID, old_values, new_values)
            if len(to_serialize) == 0:
                continue
            run_sql(
    ...
    def remove_index(self, index_id, recID_low, recID_high):
        ...
        for recID in xrange(recID_low, recID_high + 1):
            old_values = old_virtual_values.get(recID) or []
            to_serialize = remove_from_cache_for_record(
                index_name, recID, old_values)
            if len(to_serialize) == 0:
                continue
            run_sql(
                """INSERT INTO %s (id_bibrec,termlist,type)
    ...
    def update_cache_for_record(self, index_name, recID, old_values,
                                new_values):
        """
        ...
        # split old values from v.index into those with 'prefix' and those
        # without
    ...
    def insert_to_cache_for_record(self, index_name, recID, old_values,
                                   new_values):
        """
    ...
    def remove_duplicates(self, entries):
        ...
        return [
            ...
            if entry not in unique and not unique.add(entry)
        ]
...
class WordTable(AbstractIndexTable):
    ...
    def __init__(self, index_name, table_type, table_prefix="",
        ...
        """
        AbstractIndexTable.__init__(
            self, index_name, table_type, table_prefix, wash_index_terms)
        ...
        if self.stemming_language and self.table_name.startswith('idxWORD'):
            write_message('%s has stemming enabled, language %s' %
                          (self.table_name, self.stemming_language))
                ...
                for t in CFG_BIBINDEX_INDEX_TABLE_TYPE:
                    if self.table_type == CFG_BIBINDEX_INDEX_TABLE_TYPE[t]:
                        ...
                        instance = tokenizer(self.stemming_language,
                                             ...
                                             self.remove_html_markup,
                                             self.remove_latex_markup)
                        special_tags[tag] = instance.get_tokenizing_function(
                            self.table_type)
                        break
        return special_tags
    ...
    def notify_virtual_indexes(self, recID_ranges):
        """
            ...
            @type recID_ranges: list [[low_id1, high_id1],
                [low_id2, high_id2]...]
        """
        query = """INSERT INTO %s
                   (runtime, id_bibrec_low, id_bibrec_high, index_name, mode)
                   VALUES (%%s, %%s, %%s, %%s, %%s)"""
        for index_id, _ in self.virtual_indexes:
    ...
    def add_recIDs(self, recIDs, opt_flush):
        ...
        for arange in recIDs:
            ...
            while i_low <= arange[1]:
                ...
                percentage_display = get_percentage_completed(
                    records_done, records_to_go)
                task_update_progress(
                    "(%s:%s) adding recs %d-%d %s" % (
                        self.table_name,
                        self.index_name,
                        i_low, i_high,
                        percentage_display
                ...
                if flush_count >= opt_flush:
                    ...
                    flush_count = 0
                    self.log_progress(
                        time_started, records_done, records_to_go)
                # iterate:
                i_low = i_high + 1
        if flush_count > 0:
            self.put_into_db()
    ...
    def add_recID_range(self, recID1, recID2):
        ...
        if self.index_name in ('author', 'firstauthor', 'exactauthor', 'exactfirstauthor'):
            for recID in range(recID1, recID2 + 1):
                if recID not in wlist:
                    wlist[recID] = []
                wlist[recID] = list_union(
                    get_author_canonical_ids_for_recid(recID),
                    wlist[recID]
                )
        ...
        for recID in recIDs:
            ...
            if "DELETED" in self.get_field(recID, "980__c"):
                wlist[recID] = []
                write_message(
                    "... record %d was declared deleted, "
                    "removing its word list"
                    % recID,
                    verbose=9
                )
            write_message("... record %d, termlist: %s" %
                          (recID, wlist[recID]), verbose=9)
        ...
        for recID in recIDs:
            run_sql(
                """INSERT INTO %sR (id_bibrec,termlist,type)
                   VALUES (%%s,%%s,'FUTURE')"""
                % wash_table_column_name(self.table_name[:-1]),
    ...
    def log_progress(self, start, done, todo):
        ...
        write_message(
            "%d records took %.1f seconds to complete.(%1.f recs/min)"
    ...
    def del_recID_range(self, low, high):
        ...
        write_message(
            "%s fetching existing words for records #%d-#%d started"
            % (self.table_name, low, high),
            verbose=3
        )
    ...
    def check_bad_words(self):
        ...
        """
        query = """SELECT 1 FROM %sR
                   WHERE type IN ('TEMPORARY','FUTURE') LIMIT 1""" \
                % (self.table_name[:-1],)
        res = run_sql(query)
        return bool(res)
    ...
    def repair(self, opt_flush):
        ...
        query = """SELECT id_bibrec FROM %sR
                   WHERE type IN ('TEMPORARY','FUTURE')""" \
                % (self.table_name[:-1])
    ...
    def chk_recID_range(self, low, high):
        ...
        if not res:
            write_message("%s for %d-%d is in consistent state" %
                          (self.table_name, low, high))
            return  # okay, words table is consistent

        # inconsistency detected!
        write_message("EMERGENCY: %s inconsistencies detected..." %
                      self.table_name)
    ...
    def fix_recID_range(self, low, high):
        ...
        state = {}
        query = """SELECT id_bibrec,type FROM %sR
            ...
            if not 'TEMPORARY' in state[recID]:
                if 'FUTURE' in state[recID]:
                    if 'CURRENT' not in state[recID]:
                        write_message(
                            "EMERGENCY: Index record %d is "
                            "in inconsistent state. Can't repair it."
                            % recID
                        )
                        ok = 0
                    else:
                ...
                if 'FUTURE' in state[recID] and not 'CURRENT' in state[recID]:
                    ...
                    write_message("EMERGENCY: %s for %d is in inconsistent "
                                  "state. Couldn't repair it."
                                  % (self.table_name, recID),
                                  stream=sys.stderr)
                    ok = 0
...
def remove_dependent_index(virtual_indexes, dependent_index):
    ...
    for index_name in virtual_indexes:
        ...
        query = """DELETE FROM idxINDEX_idxINDEX
                   WHERE id_virtual=%s AND id_normal=%s"""
    ...
    for index_name in virtual_indexes:
        if reindex:
            ...
            for key, type_ in CFG_BIBINDEX_INDEX_TABLE_TYPE.iteritems():
                kwargs.update(
                    {'wash_index_terms': CFG_BIBINDEX_WASH_INDEX_TERMS[key]})
