...
from workflow.engine import HaltProcessing, StopProcessing, JumpTokenBack, JumpTokenForward, WorkflowError
...
class BibWorkflowEngine(GenericWorkflowEngine):
    ...
    def __init__(self, name="Default workflow", uuid=None, curr_obj=0, workflow_object=None, user_id=0, module_name="Unknown"):
        self.db_obj = None
        if isinstance(workflow_object, Workflow):
            ...
            if uuid is not None:
                self.db_obj = Workflow.query.filter(Workflow.uuid == uuid).first()
            else:
                uuid = new_uuid()

            if self.db_obj is None:
                self.db_obj = Workflow(name=name, user_id=user_id, current_object=curr_obj, module_name=module_name, uuid=uuid)
                self._create_db_obj()
    ...
    def add_log(self):
        self.log = add_log(os.path.join(CFG_BIBWORKFLOW_WORKFLOWS_LOGDIR, "workflow_%s.log" % (self.db_obj.uuid, )),
    ...
    def __getstate__(self):
        if not self._picklable_safe:
            raise cPickle.PickleError("The instance of the workflow engine cannot be serialized, "
    ...
    def __setstate__(self, state):
        if len(self._objects) < self._i[0]:
            raise cPickle.PickleError("The workflow instance inconsistent state, too few objects")
        self.__dict__ = state
        self.add_log()
    ...
    def before_processing(objects, self):
        ...
        self.log.info("[%s], %s has been started" % (str(datetime.now()), str(self.db_obj.name)))
        # 2. We want to save all the objects as version 0.
        for o in objects:
            same_workflow = o.db_obj.workflow_id and o.db_obj.workflow_id == self.db_obj.uuid
            if o.db_obj.id and same_workflow:
                # If object exists and we are running the same workflow, do nothing
                continue
            # Set the current workflow id in the object
            o.db_obj.workflow_id = self.db_obj.uuid
            if o.db_obj.id:
                # If object exists and we are running a different workflow, keep reference to old id
                o.db_obj.parent_id = o.db_obj.id
            o.save(0)
    ...
    def save(self, status=0, e=""):
        """
        Save the workflow instance to database. Just storing the necessary data. No serialization (!).

        Status: 0 - running, 1 - halted, 2 - error, 3 - finished
        """
        if not self.db_obj.uuid:
            # We do not have an ID, so we need to add ourselves (first execution).
            self._create_db_obj()
        else:
    ...
    def haltProcessing(self, msg):
        ...
        raise HaltProcessing("Processing halted at task %s with message: %s" % (self.getCurrTaskId(), msg, ))
