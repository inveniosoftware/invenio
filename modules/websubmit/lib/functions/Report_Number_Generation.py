     ...
     PARAMETERS:   edsrn: name of the file in which the reference is saved
                   ...
                   counterpath: path to the counter file note you can use: 
                                ...
                                <PA>categ</PA> to include category of the 
                                    submission
                                <PA>file:name_of_file</PA> first line of file 
                                    generated by submission
                                <PA>file*:name_of_file</PA> all the lines of
                                    a file genereated during submission, 
    ...
    if parameters['autorngen'] == "Y":
        ...
        def get_pa_tag_content(pa_content):
            ...
            return: if pa_content=yy => 4 digits year
                    ...
                    if pa_content=file:a_file => first line of file a_file
                    if pa_content=file*:a_file => all lines of file a_file,
                                                  separated by - (dash) char.
            """
            ...
            if pa_content=='yy':
                ...
                out = category
            elif pa_content[:6]=='file*:':
                if os.path.exists(os.path.join(curdir, pa_content[6:])):
                    fp = open(os.path.join(curdir, pa_content[6:]), 'r')
                    out = sep.join(map(lambda x: x.strip(), fp.readlines()))
                    fp.close()
            elif pa_content[:5]=='file:':
                if os.path.exists(os.path.join(curdir, pa_content[5:])):
                    fp = open(os.path.join(curdir, pa_content[5:]), 'r')
