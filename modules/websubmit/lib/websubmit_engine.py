...
def _convert_tag_tuple_array_to_author_dictionary(record_tag):
    ...
    for _tuple in record_tag:
        ...
        if _tuple[0] == CFG_SUBFIELD_DEFINITIONS["id"]:
            ...
            if CFG_SUBFIELFD_TO_JSON_FIELDS[
                CFG_SUBFIELD_DEFINITIONS["id"]
            ].get(retrieve_authorid_type(_tuple[1])):

                author[CFG_SUBFIELFD_TO_JSON_FIELDS[
                    CFG_SUBFIELD_DEFINITIONS["id"]
                ].get(
                    retrieve_authorid_type(_tuple[1])
                )] = retrieve_authorid_id(_tuple[1]).decode("string_escape")

        elif CFG_SUBFIELFD_TO_JSON_FIELDS.get(_tuple[0]):

            if (not _tuple[1] == " ") and _tuple[1]:
                author[
                    CFG_SUBFIELFD_TO_JSON_FIELDS.get(_tuple[0])
                ] = _tuple[1].decode("string_escape")
...
def _convert_record_authors_to_json(recid):
    ...
    if record:
        ...
        if record_authors:
            ...
            return dumps({"items": json_authors})
...
def get_authors_autocompletion(
    ...
    extra_author_sources = extra_author_sources or []
    write_file(
        "%s/EXTRA_AUTHOR_SOURCES" % curdir, "\n".join(extra_author_sources))
    ...
    return websubmit_templates.tmpl_authors_autocompletion(
        ...
        author_sources_p=bool(author_sources or extra_author_sources),
...
def get_authors_from_allowed_sources(
    ...
    extra_author_sources = ParamFromFile(
        os.path.join(
            CFG_WEBSUBMIT_STORAGEDIR,
            relative_curdir,
            "EXTRA_AUTHOR_SOURCES"
        )
    )
    extra_author_sources = filter(None, extra_author_sources.split("\n"))

    author_sources.extend(extra_author_sources)
    ...
    for author_source in author_sources:
        if author_source in extra_author_sources and result:
            # NOTE: This means that not all extra_author_sources will
            # necessarily be queried. If at least one yields results we stop
            # looking into the rest of them.
            break
        author_source = author_source.rstrip()
        if author_sources_plugins.has_key(str(author_source)):
            try:
                result.extend(
                    author_sources_plugins[author_source]["query_function"](
                        query
                    )
                )
            except:
                register_exception(
                    req=req,
                    alert_admin=True,
                    prefix="Error in executing plugin %s with globals %s" % (
                        pprint.pformat(author_source),
