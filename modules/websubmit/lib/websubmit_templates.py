...
if (typeof datum === "undefined") {
    ...
    datum = {};
    split_author = document.getElementById('author_textbox').value.split(':');
    if (split_author.length != 2){
        alert("If you wish to enter a custom author please use this format: 'Lastname, Firstname: Affiliation'");
        return;
    }
    try {
        datum['lastname'] = split_author[0].split(',')[0].trim();
        datum['firstname'] = split_author[0].split(',')[1].trim();
        datum['affiliation'] = split_author[1].trim();
    } catch(err) {
        alert("If you wish to enter a custom author please use this format: 'Lastname, Firstname: Affiliation'");
        return;
    }
    if (datum['firstname'].length < 1){
        alert("The Initial(s) of the FirstName(s) are missing.");
        return;
    }
    if (datum['lastname'].length < 2){
        alert("The LastName is too short (< 2 characters).");
        return;
    }
    if (datum['firstname'].indexOf('.') > -1 || datum['firstname'].indexOf('&') > -1 || datum['firstname'].indexOf('#') > -1){
        alert("The Initial(s) of the FirstName(s) contains an illegal character (Period or Dot (.) or Ampersand (&) or hash (#)");
        return;
    }
    if (datum['lastname'].indexOf('.') > -1 || datum['lastname'].indexOf('&') > -1 || datum['lastname'].indexOf('#') > -1){
        alert("The Initial(s) of the LastName(s) contains an illegal character (Period or Dot (.) or Ampersand (&) or hash (#)");
        return;
    }
}"""
            custom_authors_text = """
...
<script type="text/javascript">
    ...
    function importAuthorsFromInput() {
        var json = document.getElementById('authors_json_input').value;
    ...
    $(document).ready(function() {
        ...
        $('#author_textbox').on('typeahead:closed', null);
                ...
                "name": element['name'],
                "value": cgi.escape(element['value'], quote=True),
                "params": params,
