        ...
        if current_exitcode == 0:
            ...
            if current_exitcode != 0:
                ...
                if len(authorlist_record) == 1:
                    ...
                    # Replace 100,700 fields of original record with extracted fields if they are missing $i
                    if not record_get_field_values(existing_record, '100', code="i"):
                    ...
                    if not record_get_field_values(existing_record, '700', code="i"):
                        # We did not found existing $i, so replace
...
def create_authorlist_ticket(matching_fields, identifier, queue, missing_ids=False):
    """
    ...
    """
    if missing_ids:
        subject = "[OAI Harvest] Missing INSPIRE IDs for record %s" % (identifier,)
        text = """
    Harvested record with identifier %(ident)s has had its authorlist extracted and some INSPIRE Id's are missing.
