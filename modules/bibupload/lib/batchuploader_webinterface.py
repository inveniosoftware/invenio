...
class WebInterfaceBatchUploaderPages(WebInterfaceDirectory):
    ...
    def _lookup(self, component, path):
        def restupload(req, form):
            ...
            argd = wash_urlargd(form, {'callback_url': (str, None), 'nonce': (str, None), 'special_treatment': (str, None), 'priority': (int, 0)})
            return cli_upload(req, open(filepath), '--' + path[0], argd['callback_url'], argd['nonce'], argd['special_treatment'], argd['priority'])
        ...
        def legacyrobotupload(req, form):
            ...
            """
            argd = wash_urlargd(form, {'mode': (str, None), 'callback_url': (str, None), 'nonce': (str, None), 'special_treatment': (str, None), 'priority': (int, 0)})
            return cli_upload(req, form.get('file', None), argd['mode'], argd['callback_url'], argd['nonce'], argd['special_treatment'], argd['priority'])
