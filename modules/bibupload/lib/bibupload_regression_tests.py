...
#        """
#        recs = bibupload.xml_marc_to_records(testrec1_xm_to_replace)
#        err, self.recid, _ = bibupload.bibupload_records(recs, opt_mode='replace')[0]
...
class BibUploadFMTModeTest(GenericBibUploadTest):
    ...
    def tearDown(self):
        ...
        for (last_updated, value, bibformat) in self.recid76_fmts:
            run_sql("""UPDATE bibfmt SET last_updated=%s, value=%s WHERE id_bibrec=76 AND format=%s""", (last_updated, value, bibformat))
        inserted_xm = print_record(recid, 'xm')
        inserted_hm = print_record(recid, 'hm')
    ...
    def test_inserting_new_record_containing_fmt_tag(self):
        ...
        dummy, new_recid, _ = bibupload.bibupload_records(recs, opt_mode='insert')[0]
