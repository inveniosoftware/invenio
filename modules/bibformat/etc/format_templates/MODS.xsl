...
<!--
<name>MODS</name>
<description>Metadata Object Description Schema</description>
-->
<!--

This stylesheet was originally taken from:
    <http://www.loc.gov/standards/mods/v3/MARC21slim2MODS3-3.xsl>
Version modified by CDS Invenio team to include:
    <http://www.loc.gov/marcxml/xslt/MARC21slimUtils.xsl>
and to support CDS Invenio custom features.

Includes modification contributed by RERO DOC team.

This stylesheet transforms a MARCXML input into MODS output.
...
-->
  ...
  <xsl:template name="marcRecord">
    ...
    <!-- [ Contributed by RERO  20090303 -->
    <!-- define variable "subfield980__a" for document type -->
    <xsl:variable name="subfield980__a" select="datafield[@tag=980]/subfield[@code='a']"/>
    <!-- Contributed by RERO ] -->
    ...
    <xsl:for-each select="datafield[@tag='130']|datafield[@tag='240']|datafield[@tag='730'][@ind2!='2']">
    ...
    <xsl:for-each select="datafield[@tag='100']">
      <name type="personal">
        ...
        <!-- Deactivated the default "creator" role applied to field 100, as it is not applied to the 700 either -->
        <!--
                <role>
    ...
    <typeOfResource>
      <!-- [ Contributed by RERO  20090303 -->
      <!-- Separated the handling of typeOfResource, which in Invenio is not
           extracted from the leader, but rather from 980 $a $f -->
      <xsl:choose>
        ...
        <xsl:when test="not($leader)">
          <xsl:if test="$subfield980__a">
            <xsl:choose>
              <xsl:when test="$subfield980__a='BOOK'">text</xsl:when>
              <xsl:when test="$subfield980__a='DISSERTATION'">text</xsl:when>
              <xsl:when test="$subfield980__a='IMAGE'">still image</xsl:when>
              <xsl:when test="$subfield980__a='JOURNAL'">text</xsl:when>
              <xsl:when test="$subfield980__a='NEWSPAPER'">text</xsl:when>
              <xsl:when test="$subfield980__a='POSTPRINT'">text</xsl:when>
              <xsl:when test="$subfield980__a='PREPRINT'">text</xsl:when>
              <xsl:when test="$subfield980__a='REPORT'">text</xsl:when>
              <xsl:when test="$subfield980__a='SOUND'">sound recording</xsl:when>
              <xsl:when test="$subfield980__a='THESIS'">text</xsl:when>
              <xsl:when test="$subfield980__a='VIDEO'">moving image</xsl:when>
            </xsl:choose>
          </xsl:if>
        </xsl:when>
      </xsl:choose>
    ...
    <!-- [ Contributed by RERO  20090304 -->
    <!-- Added document genre, based on 980 $a-->
    <!-- See http://www.loc.gov/marc/sourcecode/genre/genrelist.html -->
    <xsl:if test="$subfield980__a">
      <!-- Apply <genre> tag using MARC genre codes -->
      <genre authority="marcgt">
        <xsl:choose>
          <xsl:when test="$subfield980__a='BOOK'">book</xsl:when>
          <xsl:when test="$subfield980__a='DISSERTATION'">thesis</xsl:when>
          <xsl:when test="$subfield980__a='JOURNAL'">journal</xsl:when>
          <xsl:when test="$subfield980__a='NEWSPAPER'">newspaper</xsl:when>
          <xsl:when test="$subfield980__a='POSTPRINT'">article</xsl:when>
          <xsl:when test="$subfield980__a='PREPRINT'">article</xsl:when>
          <xsl:when test="$subfield980__a='REPORT'">report</xsl:when>
          <xsl:when test="$subfield980__a='THESIS'">thesis</xsl:when>
          <xsl:when test="$subfield980__a='ARTICLE'">article</xsl:when>
        </xsl:choose>
      </genre>
      <!-- Detailed genre for theses and dissertations (informal terms) -->
      <xsl:if test="$subfield980__a='THESIS' or $subfield980__a='DISSERTATION'">
        <genre>
          <xsl:choose>
            <xsl:when test="$subfield980__a='THESIS'">Doctoral thesis</xsl:when>
            <xsl:when test="$subfield980__a='DISSERTATION'">Dissertation</xsl:when>
          </xsl:choose>
        </genre>
      </xsl:if>
      <!-- Detailed genre for preprints and reports (informal terms) -->
      <xsl:if test="$subfield980__a='PREPRINT' or $subfield980__a='REPORT'">
        <genre>
          <xsl:choose>
            <xsl:when test="$subfield980__a='PREPRINT'">Preprint</xsl:when>
            <xsl:when test="$subfield980__a='REPORT'">Report</xsl:when>
          </xsl:choose>
        </genre>
      </xsl:if>
      <!-- Repeat the <genre> tag using available Zotero genre codes (otherwise default to "document") -->
      <genre authority="local">
        <xsl:choose>
          <xsl:when test="$subfield980__a='BOOK'">book</xsl:when>
          <xsl:when test="$subfield980__a='DISSERTATION'">thesis</xsl:when>
          <xsl:when test="$subfield980__a='POSTPRINT'">
            <xsl:if test="datafield[@tag=980]/subfield[@code='f']">
              <xsl:choose>
                <xsl:when test="datafield[@tag=980]/subfield[@code='f']='ART_JOURNAL'">journalArticle</xsl:when>
                <xsl:when test="datafield[@tag=980]/subfield[@code='f']='ART_INPROC'">conferencePaper</xsl:when>
                <xsl:when test="datafield[@tag=980]/subfield[@code='f']='ART_INBOOK'">bookSection</xsl:when>
              </xsl:choose>
            </xsl:if>
          </xsl:when>
          <xsl:when test="$subfield980__a='PREPRINT'">document</xsl:when>
          <xsl:when test="$subfield980__a='REPORT'">report</xsl:when>
          <xsl:when test="$subfield980__a='THESIS'">thesis</xsl:when>
    ...
    <originInfo>
      ...
      <!-- [ Contributed by RERO  20090305 -->
      <!-- If the record does not contain any kind of "publisher" field (260 $b or 773 $d),
            then add the institution's name and address (919 $a $b) as <publisher> and <placeTerm>
            (this is useful namely for gray litterature such as theses, preprints and reports) -->
      <xsl:if test="not(datafield[@tag=260]/subfield[@code='b'] or datafield[@tag=773]/subfield[@code='d'])">
        <xsl:for-each select="datafield[@tag=919]">
          <publisher>
            <xsl:value-of select="subfield[@code='a']"/>
          </publisher>
          <place>
    ...
    <xsl:for-each select="datafield[@tag=245]/subfield[@code='c']">
    ...
    <xsl:for-each select="datafield[@tag=501 or @tag=502 or @tag=504 or @tag=507 or @tag=508 or  @tag=513 or @tag=514 or @tag=515 or @tag=516 or @tag=522 or @tag=524 or @tag=525 or @tag=526 or @tag=535 or @tag=536 or @tag=540 or @tag=541 or @tag=544 or @tag=545 or @tag=546 or @tag=547 or @tag=550 or @tag=552 or @tag=555 or @tag=556 or @tag=561 or @tag=562 or @tag=565 or @tag=567 or @tag=580 or @tag=581 or @tag=584 or @tag=585 or @tag=586]">
      <note>
        <xsl:call-template name="uri"/>
        <xsl:variable name="str">
          <xsl:for-each select="subfield[@code!='6' or @code!='8']">
    ...
    <xsl:for-each select="datafield[@tag=034][subfield[@code='d' or @code='e' or @code='f' or @code='g']]">
      <subject>
        <cartographics>
          <coordinates>
            <xsl:call-template name="subfieldSelect">
    ...
    <!-- [ Contributed by RERO  20090302 -->
    <!-- Prepared for handling field 695 (keywords - informal terms) -->
    <xsl:apply-templates select="datafield[@tag=695]" />
    <!-- Contributed by RERO ] -->
    <xsl:apply-templates select="datafield[653 &gt;= @tag and @tag &gt;= 600]"/>
    <xsl:apply-templates select="datafield[@tag=656]"/>
    <xsl:for-each select="datafield[@tag=752 or @tag=662]">
      <subject>
    ...
    <xsl:for-each select="datafield[@tag=045][subfield[@code='b']]">
      <subject>
        <xsl:choose>
          <xsl:when test="@ind1=2">
            ...
            <temporal encoding="iso8601" point="end">
              <xsl:call-template name="chopPunctuation">
                <xsl:with-param name="chopString">
                  <xsl:value-of select="subfield[@code='b'][2]"/>
                </xsl:with-param>
              </xsl:call-template>
            </temporal>
          </xsl:when>
    ...
    <xsl:for-each select="datafield[@tag=760]|datafield[@tag=762]">
      <relatedItem type="series">
        <xsl:call-template name="relatedItem76X-78X"/>
      </relatedItem>
    </xsl:for-each>
    <xsl:for-each select="datafield[@tag=765]|datafield[@tag=767]|datafield[@tag=777]|datafield[@tag=787]">
      <relatedItem>
        <xsl:call-template name="relatedItem76X-78X"/>
      </relatedItem>
    </xsl:for-each>
    ...
    <xsl:for-each select="datafield[@tag='010']">
      <xsl:call-template name="isInvalid">
    ...
    <xsl:for-each select="datafield[@tag='856'][subfield[@code='u']]">
      <identifier>
        <xsl:attribute name="type">
          <xsl:choose>
            <xsl:when test="starts-with(subfield[@code='u'],'urn:doi') or starts-with(subfield[@code='u'],'doi')">doi</xsl:when>
            <xsl:when test="starts-with(subfield[@code='u'],'urn:hdl') or starts-with(subfield[@code='u'],'hdl') or starts-with(subfield[@code='u'],'http://hdl.loc.gov')">hdl</xsl:when>
            <xsl:otherwise>uri</xsl:otherwise>
          </xsl:choose>
        </xsl:attribute>
        <xsl:choose>
          <xsl:when test="starts-with(subfield[@code='u'],'urn:hdl') or starts-with(subfield[@code='u'],'hdl') or starts-with(subfield[@code='u'],'http://hdl.loc.gov') ">
    ...
    <!-- 1/04 fix added $y -->
    <!-- [ Contributed by RERO  20090302 -->
    <!-- Added custom handling of the field 8564_ -->
    <xsl:for-each select="datafield[@tag=856][@ind1=4][subfield[@code='u']]">
      <location>
        <url>
    ...
    <!-- Contributed by RERO ] -->
    <!-- 1.21  tmee-->
    <xsl:for-each select="datafield[@tag=856][@ind2=1][subfield[@code='u']]">
      <relatedItem type="otherVersion">
        <location>
          <url>
  ...
  <xsl:template name="nameABCDQ">
    ...
    <xsl:call-template name="termsOfAddress"/>
    <!-- [ Contributed by RERO  20090305 -->
    <!-- Do not add author dates (100 $d, 700 $d) with <namePart type="date">, this seems to cause trouble with Zotero
        Should be restored later when Zotero is able to deal with it -->
    <!--
        <xsl:call-template name="nameDate"/>
        -->
    <!-- Contributed by RERO ] -->
  ...
  <!-- [ Contributed by RERO  20090302 -->
  <!-- Added field 695 (keywords - informal terms) -->
