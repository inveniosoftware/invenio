...
def borrower_search_result(req, column, string, redirect='no', ln=CFG_SITE_LANG):
    ...
    if len(result) == 1:
        ...
        body = bibcirculation_templates.tmpl_borrower_search_result(result=result,
                                                                    redirect=redirect,
                                                                    ln=ln)
...
def item_search_result(req, p, f, library_id, ln=CFG_SITE_LANG):
    """
    ...
    if f == 'barcode':
        ...
        if recid is None:
            infos.append(_('The barcode <strong>%s</strong> does not exist '\
                           'on BibCirculation database.') % p)
            body = bibcirculation_templates.tmpl_item_search(infos=infos, ln=ln)
        else:
...
def loan_on_desk_step1(req, key, string, ln=CFG_SITE_LANG):
    ...
    body = bibcirculation_templates.tmpl_loan_on_desk_step1(result=borrowers_list,
                                                                key=key,
                                                                string=string,
                                                                infos=infos,
                                                                ln=ln)
...
def loan_on_desk_step2(req, user_id, ln=CFG_SITE_LANG):
    ...
    body = bibcirculation_templates.tmpl_loan_on_desk_step2(user_id=user_id,
                                                            infos=infos,
                                                            ln=ln)
...
def loan_on_desk_step3(req, user_id, list_of_barcodes, ln=CFG_SITE_LANG):
    ...
    for value in list_of_barcodes:
        ...
        if recid is None:
            ...
            infos.append('You must select one borrower.')
            body = bibcirculation_templates.tmpl_loan_on_desk_step1(result=None,
                                                                    key='',
                                                                    string='',
                                                                    infos=infos,
...
def register_new_loan(req, barcode, borrower_id,
    ...
    if print_data == 'true':
        return print_new_loan_information(req)
...
def loan_return_confirm(req, barcode, ln=CFG_SITE_LANG):
    ...
    if recid is None:
        ...
        infos.append('The item with the barcode <strong>%s</strong> '\
                     'is not on loan. Please try again.' % barcode)
        body = bibcirculation_templates.tmpl_loan_return(infos=infos, ln=ln)
...
def get_borrower_loans_details(req, recid, barcode, borrower_id,
    ...
    if barcode and loan_id and recid:
        ...
        if len(queue) != 0:
            infos.append("Another user is waiting for this book <strong>" +
                         book_title_from_MARC(recid) + "</strong>." \
...
def get_item_req_historical_overview(req, recid, ln=CFG_SITE_LANG):
    ...
    body = bibcirculation_templates.tmpl_get_item_req_historical_overview(
                                                            req_hist_overview=req_hist_overview,
                                                            ln=ln)
...
def get_item_loans_historical_overview(req, recid, ln=CFG_SITE_LANG):
    ...
    body = bibcirculation_templates.tmpl_get_item_loans_historical_overview(
                                                        loans_hist_overview=loans_hist_overview,
                                                        ln=ln)
...
def update_borrower_info_step4(req, borrower_id, name, email, phone, address, mailbox,
                               ln=CFG_SITE_LANG):
    """
    Update the borrower's information.
    """
...
def update_borrower_info_step5(req, borrower_id, name, email, phone, address, mailbox,
                               ln=CFG_SITE_LANG):
    """
    Update the borrower's information.
    """
    ...
    db.update_borrower_info(borrower_id, name, email, phone, address, mailbox)
...
def add_new_copy_step5(req, barcode, library, location, collection, description,
    ...
    body = bibcirculation_templates.tmpl_add_new_copy_step5(infos=infos, recid=recid, ln=ln)
...
def update_item_info_step2(req, p, f, ln=CFG_SITE_LANG):
    ...
    body = bibcirculation_templates.tmpl_update_item_info_step2(result=result, ln=ln)
...
def update_item_info_step3(req, recid, ln=CFG_SITE_LANG):
    ...
    body = bibcirculation_templates.tmpl_update_item_info_step3(recid=recid, result=result,ln=ln)
...
def update_item_info_step6(req, tup_infos, ln=CFG_SITE_LANG):
    ...
    if is_on_loan and status == 'on shelf':
        #borrower_id = db.get_borrower_id(old_barcode)
        #borrower_name = db.get_borrower_name(borrower_id)
    ...
    if barcode != old_barcode:
        if db.barcode_in_use(barcode):
            infos.append("Item <strong>[%s]</strong> updated,  but the <strong>barcode was not \
...
def create_new_request_step1(req, borrower_id, p="", f="", search=None, ln=CFG_SITE_LANG):
    ...
    if search and p == '':
        ...
        if has_recid is None:
            infos.append('The barcode <strong>%s</strong> does not ' \
                         'exist on BibCirculation database.' % p)
            result = ''
        else:
            result = has_recid
...
def place_new_request_step1(req, barcode, recid, key, string, ln=CFG_SITE_LANG):
    ...
    if len(result) == 1:
        return place_new_request_step2(req, barcode, recid, borrowers_list[0], ln)
    else:
        body = bibcirculation_templates.tmpl_place_new_request_step1(result=borrowers_list,
                                                                     key=key,
                                                                     string=string,
                                                                     barcode=barcode,
                                                                     recid=recid,
...
def place_new_loan_step1(req, barcode, recid, key, string, ln=CFG_SITE_LANG):
    ...
    body = bibcirculation_templates.tmpl_place_new_loan_step1(result=borrowers_list,
                                                              key=key,
                                                              string=string,
                                                              barcode=barcode,
                                                              recid=recid,
...
def ill_request_details_step1(req, delete_key, ill_request_id, new_status, ln=CFG_SITE_LANG):
    ...
    if ill_request_borrower_details is None or len(ill_request_borrower_details) == 0:
    ...
    if infos == []:
        body = bibcirculation_templates.tmpl_ill_request_details_step1(
        ...
        body = bibcirculation_templates.tmpl_display_infos(infos, ln)
...
def ill_request_details_step2(req, delete_key, ill_request_id, new_status, library_id,
    ...
    db.update_ill_request(ill_request_id, library_id, request_date, expected_date,
                          arrival_date, due_date, return_date, new_status, cost_format,
                          barcode, str(library_previous_notes))
...
def register_ill_request_with_no_recid_step1(req, borrower_id, ln=CFG_SITE_LANG):
    """
    """
    id_user = getUid(req)
    (auth_code, auth_message) = is_adminuser(req)
    ...
    if borrower_id in (None, '', 'None'):
        ...
        if not key:
            ...
            if validate_date_format(period_of_interest_from) is False:
                ...
                body = bibcirculation_templates.tmpl_register_ill_request_with_no_recid_step1(
                                                                            infos=infos,
                                                                            borrower_id=None,
                                                                            admin=True,
                                                                            ln=ln)
        ...
        return register_ill_request_with_no_recid_step3(req, title, authors, place, publisher,
                                            year, edition, isbn, user_info, budget_code,
...
def register_ill_request_with_no_recid_step3(req, title, authors, place,
    ...
    if user_info is None:
        return register_ill_request_with_no_recid_step2(req, title, authors, place,
                            publisher, year, edition, isbn, budget_code, period_of_interest_from,
                            period_of_interest_to, additional_comments,
                            only_edition, 'name', None, None, ln)
...
def register_ill_request_with_no_recid_step4(req, book_info, borrower_id, request_details, ln):
...
### budget_code ###
    if db.get_borrower_data_by_id(borrower_id) == None:
        ...
        body = bibcirculation_templates.tmpl_register_ill_request_with_no_recid_step2(
                            book_info=book_info, request_details=request_details,
                            result=[], key='name', string=None, infos=infos, ln=ln)
        ...
        db.ill_register_request_on_desk(borrower_id, book_info, period_of_interest_from,
                                    period_of_interest_to, 'new', str(ill_request_notes),
    ...
    if p == '':
        ...
        if f == 'barcode':
            ...
            if recid is None:
                ...
                body = bibcirculation_templates.tmpl_register_ill_book_request_result(
                                                                    result=[recid],
                                                                    borrower_id=borrower_id,
                                                                    ln=ln)
        else:
            result = perform_request_search(cc="Books", sc="1", p=p, f=f)
            if len(result) == 0:
                return register_ill_request_with_no_recid_step1(req, borrower_id, ln)
            else:
                body = bibcirculation_templates.tmpl_register_ill_book_request_result(
                                                                        result=result,
                                                                        borrower_id=borrower_id,
...
def register_ill_book_request_from_borrower_page(req, borrower_id, ln=CFG_SITE_LANG):
    ...
    body = bibcirculation_templates.tmpl_register_ill_book_request_from_borrower_page(
                                                                        infos=infos,
                                                                        borrower_id=borrower_id,
                                                                        ln=ln)
...
### not used ###
def register_ill_book_request_from_borrower_page_result(req, borrower_id, p, f,
                                                             ln=CFG_SITE_LANG):
    """
    Search an item and return a list with all the possible results. To retrieve
    the information desired, we use the method 'perform_request_search' (from
    ...
    if f == 'barcode':
        ...
        if has_recid is None:
            infos.append('The barcode <strong>%s</strong> does not exist '\
                         'on BibCirculation database.' % p)
            body = bibcirculation_templates.tmpl_register_ill_book_request_from_borrower_page(
                                                                            infos=infos, ln=ln)
        else:
            body = bibcirculation_templates.tmpl_register_ill_book_request_from_borrower_page_result(
                result=has_recid,
                borrower_id=borrower_id,
                ln=ln)
            #return get_item_details(req, has_recid, ln=ln)
    else:
        result = perform_request_search(cc="Books", sc="1", p=p, f=f)
        if len(result) == 0:
...
def register_ill_request_from_borrower_page_step1(req, borrower_id, ln=CFG_SITE_LANG):
    ...
    body = bibcirculation_templates.tmpl_register_ill_request_from_borrower_page_step1(
                                                                        infos=infos,
                                                                        borrower_id=borrower_id,
                                                                        ln=ln)
...
def register_ill_article_request_step2(req, periodical_title, article_title, author,
                                       report_number, volume, issue, pages, year, budget_code,
                                       issn, period_of_interest_from, period_of_interest_to,
...
def register_ill_article_request_step3(req, periodical_title, title, authors,
                                       ...
                                       user_info, request_details, ln=CFG_SITE_LANG):
    ...
    if user_info is None:
        return register_ill_article_request_step2(req, periodical_title, title, authors,
                                       report_number, volume, issue, page_number, year,
                                       budget_code, issn, period_of_interest_from,
                                       period_of_interest_to,
...
def delete_copy_step2(req, barcode, ln):
    ...
    if recid:
        ...
        if db.delete_copy(barcode)==1:
            ...
            message = 'It was NOT possible to delete the copy with barcode '\
                      '<strong>%s</strong>' % (barcode)
...
def all_library_items(req, library_id, ln):
    ...
    if library_name is None:
        infos.append(_("There is no library with id %s" % (library_id)))
    else:
        copies = db.get_library_copies(library_id)
        if len(copies) == 0:
            infos.append(_("This library has no copies"))

    body = bibcirculation_templates.tmpl_all_library_items(library_id,
                                                           library_name,
                                                           copies,
                                                           infos,
                                                           ln)

    return page(title="All copies of library %s" % (library_name or ''),
                uid=id_user,
                req=req,
                body=body, language=ln,
                metaheaderadd = '<link rel="stylesheet" href="%s/img/jquery-ui.css" '\
