...
from invenio.config import CFG_SITE_LANG, CFG_PYLIBDIR
...
The following scale is should be considered by services:
  ...
  - 100: the service thinks it has found the only answer to the question. No other service
   can find an answer, and no other service will be displayed.

  - 90: The service thinks it has found the good answer, probably the one expected by
   the user. However some other services with really good relevance might still run.
  ...
  - 20: The service found some answer, but it is unlikely that it will help the user
...
What is the max distance from the most relevant answer do we still display the answer
from a service? For eg, with CFG_WEBSEARCH_SERVICE_MAX_RELEVANCE_DIFFERENCE==30 and the most relevant
service returning a relevance of 80, we would still display a service relevant at 50 or above,
but not a service with a relevance below 50
"""

CFG_WEBSEARCH_SERVICE_MIN_RELEVANCE_TO_DISPLAY = 21
"""
CFG_WEBSEARCH_SERVICE_MIN_RELEVANCE_TO_DISPLAY:
When do we consider that a service is not relevant enough to be displayed
"""
...
def get_search_services():
    ...
    if _SERVICES is None:
        _SERVICES = [plugin() for plugin in PluginContainer(os.path.join(CFG_SEARCH_SERVICES_PATH, '*Service.py'),
...
class SearchService:
    ...
    def timestamp_verifier(self):
        """
        Return the time at which your data was last updated.  If the
    ...
    def answer(self, req, user_info, of, cc, colls_to_search, p, f, search_units, ln):
        ...
        and html_string being an HTML-formatted answer.
    ...
    def get_data_cache(self, recreate_cache_if_needed=True):
        """
        Return always up-to-date data from cache. The returned value
        depends on the what has been stored with L{prepare_data_cache}.
        ...
        try:
            ...
            self.__class__.cache = DataCacher(self.prepare_data_cache, self.timestamp_verifier)
...
class ListLinksService(SearchService):
    ...
    def display_answer_helper(self, labels_and_urls, ln=CFG_SITE_LANG):
        """
        Helper function to display HTML to return an answer as HTML.
        ...
        return websearch_templates.tmpl_print_service_list_links(label,
                                                                 labels_and_urls,
                                                                 ln=ln)
