...
class Marc100700CacheTestCase(BibAuthorIDRabbitTestCase):  # TODO Refactor, create test utils.
    ...
    def setUp(self):
        ...
        marc_xml_record = self.get_marc_for_rabbit_test(opt_mode='insert',
                                                        author_name=self.author_name,
                                                        ext_id=None,
                                                        co_authors_names=self.coauthors_names)
        self.bibrec_to_test = self.get_bibrec_for_record(marc_xml_record,
                                                         opt_mode='insert')
    ...
    def test_cache_coauthor_refs(self):
        from_cache = _get_coauthor_refs_from_marc_caches_of_paper(self.bibrec_to_test)
        from_db = _get_coauthor_refs_from_db_of_paper(self.bibrec_to_test)
        self.assertEquals(from_cache, from_db)
