...
class AidPERSONIDPAPERS(db.Model):
    ...
    bibref_table = db.Column(db.Enum('100', '700', name='aidpps_bibref_table'),
...
class AidRESULTS(db.Model):
    ...
    personid = db.Column(db.String(128), primary_key=True, nullable=False,
                         index=True)
    bibref_table = db.Column(db.Enum('100', '700', name='aidres_bibref_table'),
                             primary_key=True, nullable=False, index=True)
...
class AidDENSEINDEX(db.Model):

    """Represents an AidDENSEINDEX record."""

    __tablename__ = 'aidDENSEINDEX'
    name_id = db.Column(db.Integer(10), primary_key=True, nullable=False,
                        autoincrement=False)
    person_name = db.Column(db.String(256), nullable=False)
    personids = db.Column(db.iLargeBinary, nullable=False)


class AidINVERTEDLISTS(db.Model):

    """Represents an AidINVERTEDLISTS record."""

    __tablename__ = 'aidINVERTEDLISTS'
    qgram = db.Column(db.String(4), primary_key=True, nullable=False,
                      autoincrement=False)
    inverted_list = db.Column(db.iLargeBinary, nullable=False)
    list_cardinality = db.Column(db.Integer(10), nullable=False)
