...
from invenio.modules.tags.models import WtgTAGRecord
...
def _build_page(filesystem, service):
    ...
    if service == 'dropbox':
        folder_metadata.sort(key=itemgetter(u'is_dir'), reverse=True)
    if service == 'google_drive':
        folder_metadata.sort(key=itemgetter(u'isdir'), reverse=True)
...
@event.listens_for(WtgTAGRecord, 'after_insert')
def receive_after_insert(mapper, connection, target):
    ...
    if 'files' in record:
        for rec in record['files']:
            if 'full_path' in rec:
                if target.tag.name in CFG_SERVICE_PRETTY_NAME:
                    upload(target.tag.name, rec['full_path'],
                           UPLOAD_FOLDER+rec['full_path'])
