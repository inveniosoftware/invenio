...
def fetch_groups(groups):
    """Convert possible list of candidate groups to Invenio groups.

    This filters out blacklisted (hidden) groups and tries to detect if a
    group is a mailing list or a normal group.
    """
    ...
    return dict(map(
        lambda x: (x, '@' in x and x + ' (Mailing list)' or x + ' (Group)'),
        groups
    ))
...
def account_info(remote, resp):
    ...
    res = {}
    res = _get_dict_from_response(response)
...
def account_setup(remote, token):
    ...
    if code == requests.codes.ok:
        ...
        current_user.info['group'] = fetch_groups(
            res['http://schemas.xmlsoap.org/claims/Group']
        )
    ...
    if user:
        for g in current_user.info['group'].iterkeys():
            ug = Usergroup()
