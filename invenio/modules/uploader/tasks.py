...
from .api import _translate, _run_worflow, _error_handler
...
def validate(step):
    ...
    The for the validation the `schema` information from the field definition is
    used, see `invenio.modules.jsonalchemy.jsonext.parsers.schema_parser`
    """
    def _validate(obj, eng):
        ...
        if not hasattr(obj, 'validate'):
            raise UploaderWorkflowException(step,
                    msg="An 'validate' method is needed")

        validator_errors = obj.validate()
        eng.log.info('Validation errors: %s' % (str(validator_errors), ))
        if mode.lower() == 'strict' and validator_errors:
            raise UploaderWorkflowException(step,
                    msg="One or more validation errors have occurred, please check "
                        "them or change the 'validation_mode' to 'permissive'.\n%s"
                        % (str(validator_errors), ))
...
def retrieve_record_id_from_pids(step):
    """
    Retrieve the record identifier from a obj by using all the persistent
    ...
    def _retrieve_record_id_from_pids(obj, eng):
        ...
        if not hasattr(obj, 'persistent_identifiers'):
            raise UploaderWorkflowException(step,
                    msg="An 'persistent_identifiers' method is needed")

        for pid_name, pid_values in obj.persistent_identifiers:
            ...
            for possible_pid in pid_values:
                ...
                pid = PersistentIdentifier.get(possible_pid.get('type'),
                        possible_pid.get('value'), possible_pid.get('provider'))
                if pid:
            ...
            if len(matching_recids) > 1:
                raise UploaderWorkflowException(step,
                        msg="Multiple records found in the database, %s that "
                            "match '%s'" % (repr(matching_recids), pid_name))
            elif matching_recids:
                obj['recid'] = matching_recids.pop()
...
def reserve_record_id(step):
    """
    Reserve a new record id for the current object and add it to `obj['recid']`.
    """
    ...
    def _reserve_record_id(obj, eng):
        ...
        try:
            ...
            obj['recid'] = int(pid.pid_value)
            pid.reserve()
            eng.log.info("Finish reserving a recid '%s' for the new record", (pid.pid_value, ))
...
def save_record(step):
    ...
    def _save(obj, eng):
        ...
        if not hasattr(obj, '_save'):
            raise UploaderWorkflowException(step,
                    msg="An '_save' method is needed")
        try:
            obj._save()
...
def update_pidstore(step):
    ...
    def _update_pidstore(obj, eng):
        eng.log.info('Look for PIDs inside the current record and register them in the DB')
        if not hasattr(obj, 'persistent_identifiers'):
            raise UploaderWorkflowException(step,
                    msg="An 'persistent_identifiers' method is needed")

        for pid_name, pid_values in obj.persistent_identifiers:
            eng.log.info("Found PID '%s' with values '%s'", (pid_name, pid_values))
            for pid_value in pid_values:
                pid = PersistentIdentifier.get(pid_value.get('type'),
                        pid_value.get('value'), pid_value.get('provider'))
                if pid is None:
                    pid = PersistentIdentifier.create(pid_value.get('type'),
                            pid_value.get('value'), pid_value.get('provider'))
                if pid.has_object('rec', obj['recid']):
                    continue
                pid.assign('rec', obj['recid'])
