...
def get_nb_readable_messages_for_user(uid):
    """ Get number of mails of a given user.
...
def check_if_need_to_delete_message_permanently(msg_ids):
    """Checks if a list of messages exist in anyone's inbox.

    If not, delete them permanently.
...
def check_quota(nb_messages):
    ...
    from invenio.modules.access.control import (
        acc_is_user_in_role, acc_get_role_id
    )
    no_quota_role_ids = [acc_get_role_id(role)
                         for role in CFG_WEBMESSAGE_ROLES_WITHOUT_QUOTA]
    res = {}
    for uid, n in run_sql("SELECT id_user_to, COUNT(id_user_to) \
                           FROM user_msgMESSAGE \
                           GROUP BY id_user_to \
                           HAVING COUNT(id_user_to) > %s", (nb_messages, )):
...
def get_nicknames_like(pattern):
    ...
    if pattern:
        try:
            res = run_sql("SELECT nickname \
                           FROM user WHERE nickname RLIKE %s", (pattern,))
...
def get_groupnames_like(uid, pattern):
    ...
    Will return only groups that user is allowed to see.
    """
    groups = {}
    if pattern:
        ...
        query1 = "SELECT id, name \
                  FROM usergroup \
                  WHERE name RLIKE %s AND join_policy like 'V%%' AND join_policy<>'VE'"
        try:
            res = run_sql(query1, (pattern,))
        except OperationalError:
            res = ()
        # The line belows inserts into groups dictionary
        # every tuple the database returned,
