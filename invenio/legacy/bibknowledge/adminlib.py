...
def perform_request_knowledge_base_show_dependencies(kb_id, ln=CFG_SITE_LANG,
    ...
    """
    name = bibknowledge.get_kb_name(kb_id)
...
def perform_request_verify_rule(ln, kbid, left, right, leftorright,
                                currentname, tuples):
    """
    Return a page element by which the user chooses an action.
    ...
    """
    return bibknowledge_templates.tmpl_select_rule_action(
        ln, kbid, left, right, leftorright, currentname, tuples)


def perform_update_kb_config(kb_id, field, expression, collection):
    """
    Update config by calling a db function.
    ...
    return bibknowledge.save_kb_dyn_config(kb_id, field,
