...
legacy = RegistryProxy(
    'legacy', ImportPathRegistry, initial=['invenio.legacy.*'])

doc_category = lambda namespace: RegistryProxy(
    'legacy.doc.' + namespace, PkgResourcesDirDiscoveryRegistry,
    'doc/'+namespace, registry_namespace=legacy)


def doc_category_topics(namespace):
    """Finds all topics in given category."""
    test = operator.methodcaller('endswith', '.webdoc')
    return dict(map(lambda filename: (os.path.basename(filename)[:-7],
                                      filename),
