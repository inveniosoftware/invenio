...
CKEDITOR.dialog.add( 'scientificchar', function( editor )
{
	/**
	 ...
	 */
	var dialog,
		lang = editor.lang.scientificChar;
	...
	return {
	    title : 'Select Scientific Symbol',
		minWidth : 430,
		minHeight : 280,
		...
		onLoad :  function()
		{
			var columns = this.definition.charColumns,
				...
				chars = editor.config.scientificChars;

			var charsTableLabel =  CKEDITOR.tools.getNextId() + '_scientificchar_table_label';
				...
				for ( var j = 0 ; j < columns ; j++, i++ )
				{
					if ( ( character = chars[ i ] ) )
					{
						...
						else
						{
							...
							charDesc = character; //lang[ _tmpName ] || character;
						}

						var charLabelId =  'cke_scientificchar_label_' + i + '_' + CKEDITOR.tools.getNextNumber();
			...
			html.push( '</tbody></table>', '<span id="' + charsTableLabel + '" class="cke_voice_label"> </span>' );
