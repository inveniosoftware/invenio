...
def get_record_index(record):
    ...
    for query, index in six.iteritems(
        cfg["SEARCH_ELASTIC_RECORDS_INDEX_MAPPING"]
    ):
        if Query(query).match(record.json):
...
def index_record(recid):
    ...
    index = get_record_index(record) or "records"
    es.index(
        index=index,
        doc_type='record',
        body=record.json,
...
def index_collection_percolator(name, dbquery):
    ...
    indices = set(cfg["SEARCH_ELASTIC_RECORDS_INDEX_MAPPING"].values())
...
def create_index(sender, **kwargs):
    ...
    for index in indices:
        ...
        elif "records.json" in mappings:
            mapping = json.load(open(mappings["records.json"], "r"))
        else:
            mapping = {}
        es.indices.delete(index="records-" + index, ignore=404)
        es.indices.create(index="records-" + index, body=mapping)
