...
class CacheItem(object):
    ...
    def __init__(self, metadata=None, children=None, timestamp=None, parent=None):
    ...
    def metadata(self, path):
        ...
        if not item or item.metadata is None or item.expired:
            try:
                ...
                raise RemoteConnectionError("Most probable reasons: access token has expired " + \
...
class SkyDriveFS(FS):
    ...
    def __init__(self, root=None, credentials=None, thread_synchronize=True):
        ...
        if (self._root == None or self._root== "/" or self._root == ""):
        ...
        if( self._credentials == None ):
            if( "SKYDRIVE_ACCESS_TOKEN" not in os.environ ):
                ...
                self._credentials['access_token'] = os.environ.get('SKYDRIVE_ACCESS_TOKEN')
                self._credentials['refresh_token'] = os.environ.get('SKYDRIVE_REFRESH_TOKEN', None)
                self._credentials['redirect_uri'] = os.environ.get('SKYDRIVE_REDIRECT_URI', None)
                self._credentials['client_id'] = os.environ.get('SKYDRIVE_CLIENT_ID', None)
                self._credentials['client_secret'] = os.environ.get('SKYDRIVE_CLIENT_SECRET', None)
    ...
    def _update(self, path, data):
        ...
        f = self.getinfo(path)
        return self.client.put( (f["title"], string_data), f["parent_id"], True )['id']
    ...
    def copydir(self, src, dst, overwrite=False, ignore_errors=False, chunk_size=16384):
        """
        @attention: SkyDrive doesn't support copy of folders. And to implement it 
    ...
    def makedir(self, path, recursive=False, allow_recreate=False ):
        ...
        if( parts[0] == "" ):
            ...
            parent_id = parts[0]
            if( not self.exists(parent_id) ):
                raise ResourceNotFoundError("parent with the id '%s' doesn't exist" % parent_id)
    ...
    def about(self):
        """
        This methods returns information about the current user with whose credentials 
            is the file system instantiated.
        """
        user = self.client.info("me")
        info = {}
        quota = self.client.get_quota()
        info['cloud_storage_url'] = "https://skydrive.live.com/"
        info['user_name'] = user['first_name'].capitalize() + " " + user['last_name'].capitalize()
    ...
    def _normpath(self, path):
        ...
        if( path in ("/skydrive/camera_roll", "/skydrive/my_documents", 
                     "/skydrive/my_photos, /skydrive/public_documents", "me/skydrive") ):
            return path
    ...
    def _metadata_to_info(self, metadata, localtime=False):
        ...
        isdir = "folder" in path or (metadata.get('type', None)=="folder")
        info = {
            'isdir': isdir,
